...
function isExistingOrRevisitPatient(context: any): boolean {
  const parent = (context.parent || {}) as any;
  const ctx = ((context.options && context.options.context) || {}) as any;
  return (
    ctx.isNewPatient === false ||
    Boolean(ctx.isRevisitedPatient) ||
    Boolean(ctx.patientUhid) ||
    Boolean(ctx.patientRegistrationId) ||
    Boolean(ctx.selectedRevisitedPatientData) ||
    Boolean(parent.uhid) ||
    Boolean(parent.patientRegistrationId) ||
    Boolean(parent.isRevisitedPatient) ||
    Boolean(parent.selectedRevisitedPatientData)
  );
}

function jsHealthCardNoFieldSchema(digitLength: number, arogyaCardSeries?: any[]) {
  const len = digitLength > 0 ? digitLength : DEFAULT_JS_HEALTH_CARD_DIGIT_LENGTH;
  const lengthMsg = `Health Card No. must be exactly ${len} digits`;
  return Yup.string()
    .trim()
    .when("patientType", {
      ...
      then: (schema) =>
        schema
          .required("Health Card No. is required")
          .matches(/^\d+$/, "Health Card No. must contain only digits")
          .test(
          ...
          .test(
            "js-health-card-series-private",
            "Series mismatch",
            (value, context) => {
              if (!value) return true;
              const cleanVal = value.replace(/\D/g, "");
              if (cleanVal.length !== len) return true;
              if (!arogyaCardSeries || arogyaCardSeries.length === 0) return true;

              const match = arogyaCardSeries.find((series: any) => {
              ...
              if (!match) {
                ...
                return context.createError({
                  message: `Please match the series under the assigned card series : ${listStr}`
                });
              }
              return true;
            }
...
function buildRegistrationPersonalDetailsSchema(
  ...
  return Yup.object().shape({
    ...
    jsHealthCardNo: jsHealthCardNoFieldSchema(jsHealthCardDigitLength, arogyaCardSeries),
