...
function genderToRoomUsage(g: "Male" | "Female" | "Mixed"): string {
            ...
            (apiMode ? (
              <div className="space-y-6 max-w-xl">
                <div>
                  <FormSelectField
                    ...
                    options={hardwareMasterOptions.filter(
                      (o) => !hardware.some((h) => h.hardwareFacilityId === parseInt(o.value, 10))
                    )}
                    value={newHardwareSelect || ""}
                    onChange={(value) => {
                      const selectedValue = typeof value === "string" ? value : value[0];
                      if (selectedValue) handleAddHardwareFromMaster(selectedValue);
                    }}
              ...
              <div className="space-y-6 max-w-xl">
                <div>
                  <FormSelectField
                    label="Add Hardware Equipment"
                    options={HARDWARE_OPTIONS.filter((name) => !hardware.some((h) => h.name === name)).map(
                      (name) => ({
                        label: name,
                        value: name,
                      })
                    ...
                    onChange={(value) => {
                      const selectedValue = typeof value === "string" ? value : value[0];
                      if (selectedValue) handleAddHardwareLocal(selectedValue);
                    }}
                    placeholder="Select equipment to add..."
                    mode="single"
                  />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {FACILITIES_OPTIONS.map((fac) => (
                    <button
                      ...
                      className={`flex items-center gap-3 p-4 rounded-lg border-2 text-left ${
                        facilities.some((f) => f.name === fac) ? "border-gray-900 bg-gray-50" : "border-gray-200"
                      }`}
                    >
                      <span
                        className={`flex h-5 w-5 rounded border-2 flex-shrink-0 ${
                          facilities.some((f) => f.name === fac) ? "border-gray-900 bg-gray-900" : "border-gray-300"
                        }`}
                      >
            ...
            <div className="max-w-2xl space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="font-semibold text-gray-900">{genderUsageDisplayLabel(genderUsage)}</p>
                </div>
                <div>
