...
function getPackageEndDate(packageEndDate?: string | null) {
  if (!packageEndDate) return null;
  const endDate = parseDateOnly(packageEndDate);
  endDate.setHours(0, 0, 0, 0);
  return endDate;
}

function isAfterPackageEndDate(date: Date, packageEndDate?: string | null) {
  const endDate = getPackageEndDate(packageEndDate);
...
function isDateDisabled(date: Date, packageEndDate?: string | null) {
  return isBeforeToday(date) || isAfterPackageEndDate(date, packageEndDate);
}

function getInitialSelectedDate(packageEndDate?: string | null) {
  const today = getTodayStart();
  if (isAfterPackageEndDate(today, packageEndDate)) {
    return getPackageEndDate(packageEndDate) ?? today;
  }
  return today;
}
...
export function NurseAllocationView({
  ...
  const [selectedDate, setSelectedDate] = useState(() => getInitialSelectedDate(patient.packageEndDate));
  const [calendarMonth, setCalendarMonth] = useState(() => {
    const initialDate = getInitialSelectedDate(patient.packageEndDate);
  ...
  const [assignNurse, { isLoading: isAssignNurseLoading }] = useStaffNurseAssignNurseMutation();
  ...
  const { data: nurseDropdownRes, isLoading: isNurseDropdownLoading } = useStaffNurseGetNurseDropdownQuery(
    { branchId: branchId! },
    { skip: branchId == null, refetchOnMountOrArgChange: true }
  );
  ...
  const isNextMonthDisabled = useMemo(() => {
    const packageEnd = getPackageEndDate(patient.packageEndDate);
    if (!packageEnd) return false;

    const nextMonthStart = new Date(calendarMonth.getFullYear(), calendarMonth.getMonth() + 1, 1);
    nextMonthStart.setHours(0, 0, 0, 0);
    return nextMonthStart > packageEnd;
  }, [calendarMonth, patient.packageEndDate]);

  useEffect(() => {
    const nextSelectedDate = getInitialSelectedDate(patient.packageEndDate);
    setSelectedDate(nextSelectedDate);
    setCalendarMonth(new Date(nextSelectedDate.getFullYear(), nextSelectedDate.getMonth(), 1));
  }, [patient.id, patient.packageEndDate]);
  ...
  const handleSelectDate = (date: Date) => {
    if (isDateDisabled(date, patient.packageEndDate)) return;
        ...
        <div className="mt-5 flex items-stretch overflow-hidden rounded-[16px] border border-[#E3EEE1] bg-white">
          ...
          <div className="flex flex-1 overflow-x-auto">
            {monthDays.map((day) => {
              ...
              const isDisabled = isDateDisabled(day.date, patient.packageEndDate);
          ...
          <button
            ...
            className={`flex w-10 shrink-0 items-center justify-center border-l border-[#E3EEE1] bg-white transition-colors ${
              isNextMonthDisabled
                    ...
                    {room.beds.map((bed) => (
                      <div key={bed.id} className="flex items-center gap-3">
                        <div className="shrink-0">
                          <p className="text-xs text-[#434956]">Bed</p>
