...
export function PatientVitalsTab({
  ...
  } = useStaffNurseGetLatestVitalsQuery(resolvedPatientId ?? 0, {
  ...
  const patientMetaItems = patient
    ? [
        ...
        patient.doctorName ? { label: "Doctor", value: patient.doctorName } : null,
        patient.nurseName ? { label: "Nurse", value: patient.nurseName } : null,
        patient.age ? { label: "Age", value: patient.age } : null,
  ...
  const statusBadgeVariant =
    patient?.statusLabel?.toLowerCase() === "pending" ? "warning" : "success";
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <div className="flex flex-wrap items-center gap-3">
                ...
                >
                  <h1 className="max-w-[min(100%,720px)] cursor-default truncate text-[20px] font-semibold leading-tight text-[#262D3B] md:text-[20px]">
                    {/* {patient.patientName} */}
                    {`${patient.patientTitle ? `${patient.patientTitle} ` : ""}${patient.patientName || "N/A"}`}
                  </h1>
                </Tooltip>
              ...
              {patientMetaItems.length > 0 ? (
                <p className="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 text-sm text-[#434956] text-[13px]">
