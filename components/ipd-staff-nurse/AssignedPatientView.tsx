...
function groupTherapyScheduleByDate(items: PatientTherapyScheduleItem[]) {
  const grouped = new Map<string, PatientTherapyScheduleItem[]>();

  items.forEach((item) => {
    const dateKey = item.therapyDate.split("T")[0];
...
function formatTherapyScheduleTime(session: PatientTherapyScheduleItem) {
  const start =
    session.scheduledStartTime || session.startTime || session.sessionStartTime;
  const end = session.scheduledEndTime || session.endTime || session.sessionEndTime;
  ...
  if (session.timeSlot?.trim()) return session.timeSlot.trim();
...
function buildTherapyScheduleGrid(items: PatientTherapyScheduleItem[]) {
  ...
  const cells = new Map<string, PatientTherapyScheduleItem[]>();

  items.forEach((session) => {
    const dateKey = session.therapyDate.split("T")[0];
...
function formatMedicineListStatus(medicineStatus: string | null | undefined): {
  ...
  if (normalized === "replaced" || normalized === "inactive") {
    return { label: "Replaced", tone: "danger" };
  }
  ...
  const patientOptions: LabRequestPatient[] = [
    {
      ...
      patientType: patient.patientType ?? "Normal Patient",
      age: patient.age,
    },
            ...
            {/* <div className="mt-4 overflow-hidden rounded-xl border-l-4 border-l-[#0B8C00] bg-[#0B8C000D] px-5 py-4">
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 xl:grid-cols-5">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-1.5 truncate text-sm uppercase font-semibold text-[#262D3B]">
                    {selectedPatient.patientType}
                  </p>
                </div>
              </div>
...
function mapTherapySessionToCard(
  session: PatientTherapyScheduleItem,
  ...
  return {
    ...
    statusLabel: formatTherapyStatusLabel(session.status),
...
function TherapyVitalsDialog({
  ...
  const { data: sessionVitalsRes, isLoading: isSessionVitalsLoading } =
    useStaffNurseGetPatientVitalsBySessionQuery(sessionId ?? 0, {
...
function EditMedicineDialog({
  ...
  const [editPatientMedicine, { isLoading: isEditingPatientMedicine }] = useStaffNurseEditPatientMedicineMutation();
...
function AddMedicineDialog({
  ...
  const [addPatientMedicine, { isLoading: isAddingPatientMedicine }] = useStaffNurseAddPatientMedicineMutation();
...
export function AssignedPatientView({ patient, onBack, initialTab }: AssignedPatientViewProps) {
  const { resolvedFilterBranchId } = useIPDNurseResolvedBranchId();
  const [activeTab, setActiveTab] = useState<PatientTab>(initialTab ?? "overview");
  ...
  const [createNursingNote, { isLoading: isCreatingNursingNote }] = useStaffNurseCreateNursingNoteMutation();
...
useEffect(() => {
  ...
  } = useStaffNurseGetPatientOverviewQuery(patient.id, {
    skip: !patient.id || activeTab !== "overview",
    refetchOnMountOrArgChange: true,
  });
  ...
  // const [updatePatientNurseTaskStatus] = useStaffNurseUpdatePatientNurseTaskStatusMutation();
  const [updatePatientNurseTaskStatus, { isLoading: isUpdatingNurseTaskStatus }] = useStaffNurseUpdatePatientNurseTaskStatusMutation();
  ...
  // const handleToggleNurseTask = async (taskId: number) => {
  //   try {
 ...
 const handleConfirmToggleNurseTask = async () => {
  ...
  } = useStaffNurseGetNursingNoteListQuery(nursingNoteListParams, {
  ...
  return (
    <div className="space-y-5">
      ...
      {activeTab === "overview" ? (
        ...
        // <div className="grid grid-cols-1 gap-5 xl:grid-cols-[minmax(280px,1fr)_700px_minmax(280px,1fr)]">
              ...
              {isPatientNurseTasksLoading ? (
                ...
                <div className="space-y-3 h-[400px] overflow-y-auto overflow-x-hidden">
                  {nursingTasks.map((task) => (
                    <div
                      key={task.id}
                      className={`rounded-[14px] border bg-white px-4 py-3 me-2 ${
                        task.completed ? "border-[#0B8C0033]" : "border-[#FED7AA]"
                      }`}
                      style={{
                        borderLeftWidth: "4px",
                      ...
                      <div className="flex items-center justify-between gap-3">
                        <div className="flex min-w-0 items-center gap-3">
                          <input
                            ...
                            className="h-4 w-4 cursor-pointer shrink-0 rounded border-[#D1D5DB] accent-[#0B8C00]"
                          />
                              ...
                              {/* <div className="space-y-2"> */}
                                ...
                                {sessions.map((session) => (
                                  <TherapyScheduleCardView
                                    ...
                                    onViewVitals={() => {
                                      setTherapyVitalsSessionId(session.sessionId);
