...
interface MedicineCardProps {
    ...
    titleIconSrc?: string;
    titleIconAlt?: string;
    items: MedicineCardItem[];
    className?: string;
}

export function MedicineCard({
    title = "Medicine",
    titleIconSrc = "/icons/medicons.svg",
                ...
                <div>
                    <div className="bg-white mb-4 ">
                        {items.map((item, index) => (
                            <div key={`${item.name}-${index}`} className="flex items-center gap-3 py-2 mb-3 last:mb-0">
                                <div>
                                    <div className="flex justify-center items-center w-9 h-9 bg-[rgba(11,140,0,0.05)] shadow-[0px_6px_30px_rgba(0,0,0,0.08)] rounded-full">
                                        <Image
                                            src={item.iconSrc ?? "/icons/medicons.svg"}
                                            alt={item.iconAlt ?? "Medical Icon"}
                                            width={20}
                                            height={20}
                                        />
                                    </div>
                                ...
                                <div>
                                    ...
                                    <p className="font-medium text-[10px] leading-[120%] text-[#434956]">{item.description}</p>
                                </div>
                            </div>
                        ))}
                    </div>
