...
export default function DoctorListPage() {
    ...
    const doctorPerm = usePermission("Settings", { subModule: "users" });
    ...
    const handleTabChange = (value: string) => {
        ...
        if (route && route !== "/settings/doctor") {
            router.push(route);
        }
    };

    const goView = (row: ApiDoctorListItem) =>
        router.push(`/settings/doctor/${row.id}?branchId=${row.branchId}`);
    const goCredentials = (row: ApiDoctorListItem) =>
        router.push(`/settings/doctor/${row.id}?branchId=${row.branchId}&section=credentials`);
    const goEdit = (row: ApiDoctorListItem) =>
        router.push(`/settings/doctor/${row.id}/edit?branchId=${row.branchId}`);
                ...
                <div className="space-y-8">
                    ...
                    <ListBorder as="section" className="px-4 py-4">
                        <div className="mb-4 w-full max-w-[600px]">
                            <Tabs options={panelTabOptions} value="doctors" onChange={handleTabChange} />
                        </div>
                                ...
                                {doctorPerm.canAdd && (
                                    <button
                                        ...
                                        onClick={() =>
                                            router.push(
                                                filterBranchId != null
                                                    ? `/settings/doctor/add?branchId=${filterBranchId}`
                                                    : "/settings/doctor/add"
                                            )
                                        }
                                    >
