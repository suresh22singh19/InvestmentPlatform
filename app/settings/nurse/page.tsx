...
export default function NurseListPage() {
    ...
    const nursePerm = usePermission("Settings", { subModule: "users" });
    // debugger;
    ...
    const handleTabChange = (value: string) => {
        ...
        if (route && route !== "/settings/nurse") {
            router.push(route);
        }
    };

    const goView = (row: ApiNurseListItem) =>
        router.push(`/settings/nurse/${row.id}?branchId=${row.branchId}`);
    const goCredentials = (row: ApiNurseListItem) =>
        router.push(`/settings/nurse/${row.id}?branchId=${row.branchId}&section=credentials`);
    const goEdit = (row: ApiNurseListItem) =>
        router.push(`/settings/nurse/${row.id}/edit?branchId=${row.branchId}`);
            ...
            <AppShell>
                <div className="space-y-8">
                    <div className="flex items-start justify-between">
                        <PageHeading title="Nurses" />
                    </div>

                    <ListBorder as="section" className="px-4 py-4">
                        <div className="mb-4 w-full max-w-[600px]">
                            <Tabs options={panelTabOptions} value="nurses" onChange={handleTabChange} />
                        </div>
                            ...
                            <div className="flex flex-wrap items-center gap-3">
                                ...
                                {nursePerm.canAdd && (
                                    <button
                                        ...
                                        onClick={() => router.push("/settings/nurse/add")}
                                    >
