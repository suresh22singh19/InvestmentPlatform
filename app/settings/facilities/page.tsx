...
const HARDWARE_FACILITIES_TYPE = "facility" as const;
...
const STORAGE_KEY = "facilities-master-page-state";
...
export default function FacilitiesPage() {
  const facilitiesPermission = usePermission("settings", { subModule: "facilities" });
  const canView = facilitiesPermission.canView;
  const canAdd = facilitiesPermission.canAdd;
  const canEdit = facilitiesPermission.canEdit;
  const canDelete = facilitiesPermission.canDelete;
  ...
  const handleDelete = async (row: RowItem) => {
    ...
    try {
      ...
      let errorMsg = "Failed to delete facility. Please try again.";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(`Failed to ${dialogMode === "add" ? "create" : "update"} facility:`, error);

      let errorMsg = `Failed to ${dialogMode === "add" ? "create" : "update"} facility. Please try again.`;
    ...
    <AppShell>
      <div className="space-y-8">
        <div className="flex items-start justify-between">
          <PageHeading title="Facility Master" />
        </div>
