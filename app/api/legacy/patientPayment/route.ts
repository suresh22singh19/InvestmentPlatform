...
export async function GET(request: NextRequest) {
    try {
        ...
        const patientId = (q.get("patientId") ?? "").trim();
        const type = (q.get("type") ?? "").trim();
        const body = { patientId, type };

        const url = `${LEGACY_PROJECT_API_BASE_URL}${LEGACY_PROJECT_ENDPOINTS.patientPayment}`;
        ...
        return Response.json(
            {
                status: false,
                message: "Failed to fetch patient payment details",
