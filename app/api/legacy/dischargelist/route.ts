...
export async function GET(request: NextRequest) {
    try {
        ...
        const body = {
            ...
            startDate: pick("startDate", "2026-03-01"),
            endDate: pick("endDate", "2026-04-08"),
            limit: Number(pick("limit", "10")) || 10,
            page: Number(pick("page", "1")) || 1,
        };

        const url = `${LEGACY_PROJECT_API_BASE_URL}${LEGACY_PROJECT_ENDPOINTS.dischargeList}`;
        ...
        return Response.json(
            {
                status: false,
                message: "Failed to fetch legacy discharge list",
