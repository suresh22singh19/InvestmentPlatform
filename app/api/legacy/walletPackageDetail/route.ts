...
export async function GET(request: NextRequest) {
    try {
        ...
        const packageId = Number(q.get("packageId") ?? 0) || 0;
        const body = { packageId };

        const url = `${LEGACY_PROJECT_API_BASE_URL}${LEGACY_PROJECT_ENDPOINTS.walletPackageDetail}`;
        ...
        return Response.json(
            {
                status: false,
                message: "Failed to fetch wallet package detail",
