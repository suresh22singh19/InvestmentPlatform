...
export async function GET(request: NextRequest) {
    try {
        ...
        const paymentId = Number(q.get("paymentId") ?? 0) || 0;
        const body = { paymentId };

        const url = `${LEGACY_PROJECT_API_BASE_URL}${LEGACY_PROJECT_ENDPOINTS.walletPackagePayment}`;
        ...
        return Response.json(
            {
                status: false,
                message: "Failed to fetch wallet package payment",
