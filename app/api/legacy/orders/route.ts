...
export async function GET(request: NextRequest) {
    try {
        ...
        const body = {
            patientId: q.get("patientId")?.trim() || "",
            uhid: q.get("uhid")?.trim() || "",
            saleType: q.get("saleType")?.trim() || "service",
            orderId: q.get("orderId")?.trim() || "",
        };

        const url = `${LEGACY_PROJECT_API_BASE_URL}${LEGACY_PROJECT_ENDPOINTS.orders}`;
