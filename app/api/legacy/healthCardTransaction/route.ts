...
export async function GET(request: NextRequest) {
    try {
        ...
        const uhid = (q.get("uhid") ?? "").trim();
        const body = { uhid };

        const url = `${LEGACY_PROJECT_API_BASE_URL}${LEGACY_PROJECT_ENDPOINTS.healthCardTransaction}`;
        ...
        return Response.json(
            {
                status: false,
                message: "Failed to fetch health card transactions",
