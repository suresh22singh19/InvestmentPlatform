...
import { ipdRegistrationClinicSchema, type RegistrationPersonalDetailsFormValues } from "@/lib/validation/registrationSchemas";
...
// LocalStorage key for duplicate exception patients (IPD Clinic)
const DUPLICATE_EXCEPTION_PATIENTS_IPD_KEY = "duplicateExceptionPatientsIPDClinic";
...
export default function IPDRegistrationPage() {
    const formType: "ipd-clinic" = "ipd-clinic";
    const [currentStep, setCurrentStep] = useState(0); // 0 = Personal, 1 = Vitals
    ...
    // Two steps for IPD Registration Clinic
    ...
    const formik = useFormik<RegistrationPersonalDetailsFormValues>({
        initialValues,
        validationSchema: ipdRegistrationClinicSchema,
    ...
    // Update duplicate exception patient status in localStorage (IPD)
    const updateDuplicateExceptionPatientStatus = useCallback((contactNo: string, patientName: string, status: "approved" | "rejected") => {
        if (typeof window === "undefined") return;

        try {
            const existing = getDuplicateExceptionPatients();
            console.log(`[IPD Registration Clinic] Looking for patient: "${patientName}" (${contactNo}) with status: ${status}`);
            console.log(`[IPD Registration Clinic] Existing patients in localStorage:`, existing);
            ...
            const updated = existing.map(patient => {
                ...
                if (patientContactNo === normalizedContactNo &&
                    ...
                    console.log(`[IPD Registration Clinic] ✅ Match found! Updating patient status: ${patient.patientName} (${patient.contactNo}) -> ${normalizedStatus}`);
            ...
            if (wasUpdated) {
                ...
                setDuplicateExceptionPatients([...updated]);
                console.log(`[IPD Registration Clinic] ✅ Status updated successfully. New state:`, updated);
            } else {
                console.warn(`[IPD Registration Clinic] ❌ No matching patient found for: "${normalizedPatientName}" (${normalizedContactNo})`);
                console.log(`[IPD Registration Clinic] Existing patients for comparison:`, existing.map(p => ({ name: p.patientName, contact: p.contactNo, status: p.status })));
            }
        } catch (error) {
            console.error("[IPD Registration Clinic] Failed to update duplicate exception patient status:", error);
        }
    }, [getDuplicateExceptionPatients]);
    ...
    // Listen for duplicate number permission updates via socket (IPD)
    useEffect(() => {
        const unsubscribe = onDuplicateNumberPermissionUpdate((socketData: any) => {
            console.log("[IPD Registration Clinic] Duplicate number permission update received:", socketData);
            // Socket data structure: { message: "...", data: { contactNo, patientName, status, ... } }
            // Extract the nested data object
            const data = socketData?.data || socketData;
            console.log("[IPD Registration Clinic] Extracted data:", data);
            ...
            if (data?.contactNo && data?.patientName && data?.status) {
                ...
                console.log("[IPD Registration Clinic] Normalized status:", normalizedStatus);
                console.log("[IPD Registration Clinic] Full socket data:", JSON.stringify(socketData, null, 2));

                if (normalizedStatus === "approved" || normalizedStatus === "rejected") {
                    console.log("[IPD Registration Clinic] Updating status for:", data.patientName, data.contactNo, "->", normalizedStatus);
                    updateDuplicateExceptionPatientStatus(data.contactNo, data.patientName, normalizedStatus as "approved" | "rejected");
                } else {
                    console.warn("[IPD Registration Clinic] Invalid status received:", normalizedStatus);
                }
            } else {
                console.warn("[IPD Registration Clinic] Missing required fields in socket data:", data);
                console.log("[IPD Registration Clinic] Full socket data structure:", JSON.stringify(socketData, null, 2));
            }
        });
        return unsubscribe;
    ...
    const handleAddNewPatient = useCallback(() => {
        ...
        setSelectedPreBookingId(null);
        setSelectedPreBooking(null);
    }, [formik, selectedApprovedPatientId, selectedPreBookingId, isRevisitedPatient, currentStep, currentPendingRegistrationId, savePendingRegistration, initialValues]);
    ...
    const handleNextStep = useCallback(() => {
        ...
        if (currentStep === 0) {
            ...
            // On Vitals step (final step), save and show success
            if (patientName && patientName.toLowerCase() !== "unknown patient") {
                savePendingRegistration(formik.values, currentStep, currentPendingRegistrationId);
            }
    ...
    }, [formik, currentStep, currentPendingRegistrationId, savePendingRegistration, initialValues]);
        ...
        <AppShell>
            <div className="flex justify-between items-center">
                <div ref={registrationHeadingRef} className="prebooking-icon flex items-center gap-3 mb-6">
                    ...
                    <PageHeading title="IPD Registration Clinic" />
                </div>
                <div>
